...
exports.signupUser = async (req, res) => {
    try {
        ...
        const user = new userSchema({ username, email, password: hashedPassword, role });
        await user.save();
        const userToken = jwt.sign({ username, role }, SECRET_KEY, { expiresIn: '30d' });

        res.status(201).json({ message: 'User created successfully', userToken, role: user.role });
    } catch (error) {
...
exports.loginUser = async (req, res) => {
    try {
        ...
        const userToken = jwt.sign({ username, role }, SECRET_KEY, { expiresIn: '30d' });
        res.status(201).json({ message: 'Logged in successfully', userToken, role });
    } catch (error) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
...
exports.updateUserDetails = async (req, res) => {
    try {
        ...
        const token = jwt.sign(updatedUser.toJSON(), SECRET_KEY, { expiresIn: '30d' });

        res.status(200).json({ message: 'User details updated successfully', token, updatedUser });
